...
void EncryptoMon::decryptPokemon(Pokemon &pokemon) { encryptPokemon(pokemon); }
...
uint8_t *EncryptoMon::getBlockIds(Pokemon &pokemon) const {
    ...
    uint8_t *blockIds =
        new uint8_t[4]{blockAPositions[order], blockBPositions[order], blockCPositions[order], blockDPositions[order]};
    return blockIds;
}
...
uint8_t EncryptoMon::getBlockOrder(Pokemon &pokemon) const { return ((pokemon.pid & 0x3E000) >> 13) % 24; }
...
void EncryptoMon::setChecksum(Pokemon &pokemon) { pokemon.checksum = calculateChecksum(pokemon); }

void EncryptoMon::setChecksum(Pokemon &pokemon, uint16_t checksum) { pokemon.checksum = checksum; }

uint16_t EncryptoMon::getChecksum(Pokemon &pokemon) const { return pokemon.checksum; }
...
void EncryptoMon::setMove(Pokemon &pokemon, uint16_t move, size_t index) {
    if (index >= 4)
        throw std::out_of_range("Index out of range for moveset array");

    BlockB *blockB = getBlockBPreShuffle(pokemon);
    blockB->moveset[index] = move;
}
...
void EncryptoMon::setMovePP(Pokemon &pokemon, uint16_t pp, size_t index) {
    if (index >= 4)
        throw std::out_of_range("Index out of range for moveset array");

    BlockB *blockB = getBlockBPreShuffle(pokemon);
    blockB->movePP[index] = pp;
}
...
bool EncryptoMon::isNicknameValid(Pokemon &pokemon, uint16_t minCharID, uint16_t maxCharID, uint8_t maxLen) const {
    uint16_t nickname[11];
    getNickname(pokemon, nickname);
    for (int i = 0; i < 11; i++) {
        if (nickname[i] == 0xFFFF)
            break;
        if (i >= maxLen || nickname[i] < minCharID || nickname[i] > maxCharID)
            return false;
    }
    return true;
}

bool EncryptoMon::isOTNameValid(Pokemon &pokemon, uint16_t minCharID, uint16_t maxCharID, uint8_t maxLen) const {
    uint16_t otName[8];
    getOTName(pokemon, otName);
    for (int i = 0; i < 8; i++) {
        if (otName[i] == 0xFFFF)
            break;
        if (i >= maxLen || otName[i] < minCharID || otName[i] > maxCharID)
            return false;
    }
    return true;
}